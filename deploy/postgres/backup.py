...
def main():
    ...
    if local:
        ...
        repo='\n'.join(['repo1-type=s3','repo1-s3-endpoint=oss-cn-shanghai.aliyuncs.com','repo1-s3-region=cn-shanghai','repo1-s3-bucket=oss-pai-k0brz1afnz8dtbcgt0-cn-shanghai','repo1-s3-uri-style=host','repo1-path=/sentry/postgresql/'+stage+'/'+name])
    ...
    for value in secrets: output=output.replace(value,'[redacted]')
    if args[0]=='info' and result.returncode==0: print(result.stdout.strip())
    else: print(json.dumps({'time':time.time(),'event':'backup_command','command':args[0],'ok':result.returncode==0,'detail':output[-3000:]},ensure_ascii=False))
